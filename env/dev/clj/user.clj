(ns user
  ...
  (:require
   ...
   [ozimos.vend.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (ozimos.vend.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (ozimos.vend.config/system-config {:profile :test})
                                  (ig/prep)))))
...
(comment
  ...
  *e
  ...
  (migratus.core/create (:db.sql/migrations state/system) "add-vends-table")
